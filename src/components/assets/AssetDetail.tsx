...
import { useGetApiV1AssetsId, useGetApiV1Attachments } from '@generated/api'
...
const AssetDetail = ({ assetId }: AssetDetailProps) => {
  ...
  const { data: attachmentsData } = useGetApiV1Attachments({ targetId: assetId, targetType: 'Asset' }, {
    ...
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 3 }}>
          {[1, 2, 3].map(i => <Skeleton key={i} height={120} variant='rounded' />)}
        </Box>
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 1.5 }}>
            ...
            {asset.location && (
              <>
                ...
                <Typography variant='body2' color='text.secondary'>
                  Location: <Box component='span' fontWeight={500} color='text.primary'>{asset.location}</Box>
                </Typography>
              </>
            )}
          </Box>
      ...
      <Card sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
        ...
        <Box sx={{ px: 3, py: 2.5, borderBottom: '1px solid', borderColor: 'divider', display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Typography variant='subtitle1' fontWeight={700}>Asset Overview</Typography>
        </Box>
        ...
        <Box sx={{ p: 3, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 3 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75, position: 'relative', py: 1 }}>
            ...
            <Typography variant='caption' color='text.disabled'>Purchase price on record</Typography>
          </Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75, position: 'relative', py: 1 }}>
            ...
            <Typography variant='body2' color='text.secondary' fontWeight={500}>Days in Use</Typography>
            <Typography variant='h4' fontWeight={700} letterSpacing={-0.5} sx={{ position: 'relative', zIndex: 1 }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75, position: 'relative', py: 1 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              {warrantyIsActive && (
                <Box sx={{ position: 'relative', width: 12, height: 12, flexShrink: 0 }}>
                  <Box sx={{
                    position: 'absolute', inset: 0, borderRadius: '50%', bgcolor: '#10b981',
                    opacity: 0.7,
            ...
            <Typography variant='caption' color='text.disabled'>
              {asset.warrantyExpiryDate
                ? (warrantyIsActive ? `Expires in ${warrantyMonths} months` : `Expired ${formatDate(asset.warrantyExpiryDate)}`)
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, lg: 5 }}>
          <Card sx={{ border: '1px solid', borderColor: 'divider', height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Box sx={{ px: 3, py: 2.5, borderBottom: '1px solid', borderColor: 'divider', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Typography variant='subtitle1' fontWeight={700}>Technical Specifications</Typography>
              {specsEntries.length > 8 && (
                <Typography variant='caption' color='primary.main' sx={{ cursor: 'pointer', fontWeight: 600, '&:hover': { textDecoration: 'underline' } }}>
                  View All
                </Typography>
              )}
            </Box>
            ...
            <Box sx={{ p: 3, flex: 1 }}>
              {specsEntries.length > 0 ? (
                <Box sx={{ display: 'grid', gridTemplateColumns: '42% 1fr', rowGap: 0 }}>
                      ...
                      >
                        {key.toLowerCase().includes('serial') ? (
                          <Box component='span' sx={{
                            ...
                            bgcolor: 'action.selected', px: 1, py: 0.25, borderRadius: 0.75
                          }}>
                            {String(value)}
                          </Box>
                        ) : (
                  ...
                  {asset.brandName && (
                    <>
                      <Typography variant='body2' color='text.secondary' sx={{ py: 1.75, borderBottom: '1px solid', borderColor: 'divider', pr: 2 }}>Brand</Typography>
                      <Typography variant='body2' fontWeight={500} sx={{ py: 1.75, borderBottom: '1px solid', borderColor: 'divider' }}>{asset.brandName}</Typography>
                    </>
                  )}
                  {asset.purchaseDate && (
                    <>
                      <Typography variant='body2' color='text.secondary' sx={{ py: 1.75, pr: 2 }}>Purchase Date</Typography>
                      <Typography variant='body2' fontWeight={500} sx={{ py: 1.75 }}>{formatDate(asset.purchaseDate)}</Typography>
                    </>
                  )}
                </Box>
              ) : (
                <Box>
                  ...
                  {(() => {
                    const rows: Array<{ label: string; value: React.ReactNode }> = [
                      { label: 'Status', value: <Chip label={status.label} size='small' sx={{ height: 20, fontSize: 11, fontWeight: 700, bgcolor: alpha(status.hex, 0.12), color: status.hex, border: `1px solid ${alpha(status.hex, 0.3)}` }} /> },
                      ...(asset.brandName ? [{ label: 'Brand', value: asset.brandName }] : []),
                      ...(asset.location ? [{ label: 'Location', value: asset.location }] : []),
                      ...(asset.initialCost ? [{ label: 'Initial Cost', value: formatCurrency(asset.initialCost) }] : []),
                      ...(asset.purchaseDate ? [{ label: 'Purchase Date', value: formatDate(asset.purchaseDate) }] : []),
                      ...(asset.warrantyExpiryDate ? [{ label: 'Warranty Expiry', value: formatDate(asset.warrantyExpiryDate) }] : [])
                    ]
                        ...
                        {rows.map(({ label, value }, i) => (
                          <>
                            <Typography
                              ...
                              variant='body2'
                              color='text.secondary'
                              sx={{ py: 1.75, borderBottom: i < rows.length - 1 ? '1px solid' : 'none', borderColor: 'divider', pr: 2 }}
                            >
                              {label}
                            </Typography>
                            <Box
                              key={`v-${i}`}
                              sx={{ py: 1.75, borderBottom: i < rows.length - 1 ? '1px solid' : 'none', borderColor: 'divider', display: 'flex', alignItems: 'center' }}
                            >
                              {typeof value === 'string' ? (
            ...
            {/* Image preview */}
            <Box sx={{ mx: 3, mb: 3, p: 2, borderRadius: 2, border: '1px solid', borderColor: 'divider', bgcolor: 'action.hover', display: 'flex', alignItems: 'center', gap: 2 }}>
              <Box sx={{
                width: 56, height: 56, borderRadius: 1.5, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider',
                display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden'
              }}>
                {primaryImage?.downloadUrl ? (
                  <Box component='img' src={primaryImage.downloadUrl} alt='Primary' sx={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                  <i className='tabler-photo' style={{ fontSize: 28, opacity: 0.3 }} />
                )}
              </Box>
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Typography variant='body2' fontWeight={500}>Primary Image</Typography>
                <Typography variant='caption' color='text.disabled'>
        ...
        <Grid size={{ xs: 12, lg: 7 }}>
          <Card sx={{ border: '1px solid', borderColor: 'divider', height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Box sx={{ px: 3, py: 2.5, borderBottom: '1px solid', borderColor: 'divider', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Typography variant='subtitle1' fontWeight={700}>Maintenance Timeline</Typography>
                  ...
                  <Box key={event.id} sx={{ display: 'flex', gap: 2, position: 'relative', zIndex: 1 }}>
                    ...
                    >
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                        ...
                        <Typography variant='caption' color='text.disabled' fontWeight={500} sx={{ whiteSpace: 'nowrap', ml: 2 }}>
                          {event.date}
                        </Typography>
                      </Box>
      ...
      <Box>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          <Typography variant='h6' fontWeight={700}>Attachments &amp; Receipts</Typography>
          <Typography variant='body2' color='text.secondary'>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 2 }}>
          {attachments.map(att => (
            ...
            >
              {att.contentType?.startsWith('image/') ? (
                <Box component='img' src={att.downloadUrl ?? ''} alt={att.fileName ?? ''} sx={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                <>
                  <i className='tabler-file-description' style={{ fontSize: 36, opacity: 0.3 }} />
                  <Box sx={{ px: 2, pb: 2, textAlign: 'center' }}>
                    <Typography variant='caption' fontWeight={600} sx={{ display: 'block', lineHeight: 1.3 }}>{att.fileName}</Typography>
                    <Typography variant='caption' color='text.disabled'>
              ...
              <Box className='doc-overlay' sx={{
                position: 'absolute', top: 6, right: 6,
                bgcolor: 'rgba(0,0,0,0.5)', borderRadius: 0.75, p: 0.5,
          ...
          {attachments.length === 0 && (
            ...
            >
              ...
              <Typography variant='caption' color='text.disabled'>No attachments</Typography>
            </Box>
          )}
        </Box>
      </Box>
