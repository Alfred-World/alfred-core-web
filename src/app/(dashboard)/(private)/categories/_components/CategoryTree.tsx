...
const CategoryTree = ({ selectedId, onSelect, onCreateNew }: CategoryTreeProps) => {
  ...
  const apiErrorMessage = useMemo(() => {
    const error = countsError || treeError
    ...
    return apiError.errors?.[0]?.message || 'Failed to load categories'
  }, [countsError, treeError])

  // ─── Error handling ──────────────────────────────────────────────────────────
  useEffect(() => {
    if (isCountsError || isTreeError) {
      toast.error(apiErrorMessage || 'Failed to load categories')
    }
  }, [isCountsError, isTreeError, apiErrorMessage])
