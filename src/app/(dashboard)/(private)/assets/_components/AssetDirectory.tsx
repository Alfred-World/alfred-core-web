...
const AssetDirectory = () => {
  ...
  const apiErrorMessage = useMemo(() => {
    const error = totalError || activeError || brokenError || dataError
    ...
    return apiError.errors?.[0]?.message || 'Failed to load assets'
  }, [totalError, activeError, brokenError, dataError])

  // ─── Error handling ──────────────────────────────────────────────────────────
  useEffect(() => {
    if (isTotalError || isActiveError || isBrokenError || isDataError) {
      toast.error(apiErrorMessage || 'Failed to load assets')
    }
  }, [isTotalError, isActiveError, isBrokenError, isDataError, apiErrorMessage])
