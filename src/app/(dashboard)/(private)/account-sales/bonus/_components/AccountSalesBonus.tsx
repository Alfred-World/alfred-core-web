...
const TiersTab = () => {
  ...
  useEffect(() => {
    if (tiersQuery.isSuccess) {
      setDrafts(
        originalTiers.map(t => ({ id: t.id!, orderThreshold: t.orderThreshold ?? 0, bonusAmount: t.bonusAmount ?? 0 }))
      )
    }
      ...
      for (const draft of activeDrafts) {
        if (draft.isNew) {
          await createMutation.mutateAsync({
            data: { orderThreshold: draft.orderThreshold, bonusAmount: draft.bonusAmount }
          })
        } else {
          ...
          if (orig && (orig.orderThreshold !== draft.orderThreshold || orig.bonusAmount !== draft.bonusAmount)) {
            await updateMutation.mutateAsync({
              tierId: draft.id,
              data: { orderThreshold: draft.orderThreshold, bonusAmount: draft.bonusAmount, isActive: true }
            })
          }
        }
      }
  ...
  const maxPotentialPayout =
    activeDraftsSorted.length > 0 ? Math.max(...activeDraftsSorted.map(d => d.bonusAmount || 0)) : 0
          ...
          <Box sx={{ px: 4, pt: 4, pb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <Box>
              <Typography
                variant='h5'
                fontWeight={700}
                sx={{ letterSpacing: '-0.5px', mb: 0.5, color: 'text.primary' }}
              >
                Configure Bonus Tiers
              </Typography>
              <Typography variant='body2' sx={{ color: 'text.secondary' }}>
                Define performance thresholds and incentive payouts for the current sales cycle.
              </Typography>
            </Box>
            <IconButton
              size='small'
              sx={{ color: 'text.disabled', '&:hover': { color: 'text.primary', bgcolor: 'action.hover' } }}
            >
                ...
                {tiersQuery.isLoading ? (
                  ...
                  <>
                    {activeDrafts.map((tier, index) => (
                      ...
                      >
                        ...
                        <IconButton
                          ...
                          onClick={() =>
                            setDrafts(prev => prev.map(d => (d.id === tier.id ? { ...d, isDeleted: true } : d)))
                          }
                        >
                    ...
                    <Button
                      onClick={() => {
                        const nextThreshold =
                          activeDraftsSorted.length > 0
                            ? Math.max(...activeDraftsSorted.map(d => d.orderThreshold)) + 1
                            : 1

                        setDrafts(prev => [
                          ...prev,
                          { id: `new_${Date.now()}`, orderThreshold: nextThreshold, bonusAmount: 0, isNew: true }
                        ])
                      }}
                      sx={{
                        py: 2,
            ...
            <Box>
              <Stack spacing={3}>
                ...
                >
                  ...
                  <Typography
                    variant='body2'
                    sx={{ color: 'text.secondary', lineHeight: 1.6, mb: 3, position: 'relative', zIndex: 1 }}
                  >
                    Based on last month&apos;s performance, adding a tier at{' '}
                    <Typography component='span' fontWeight={700} color='text.primary'>
                      35 orders
                    </Typography>{' '}
                    could increase total volume by{' '}
                    <Typography component='span' fontWeight={700} color='#818cf8'>
                      14%
                    </Typography>
                    .
                  </Typography>
                  ...
                  <Box
                    sx={{ display: 'flex', justifyContent: 'space-between', mb: 1.5, position: 'relative', zIndex: 1 }}
                  >
                    <Typography variant='caption' sx={{ color: 'text.disabled' }}>
                      Max potential payout
                    </Typography>
                    <Typography variant='caption' fontWeight={700} sx={{ color: 'text.primary' }}>
                      {maxPotentialPayout.toLocaleString('en-US')} VND
                    </Typography>
                  </Box>
                  <Box
                    sx={{ display: 'flex', justifyContent: 'space-between', mb: 3, position: 'relative', zIndex: 1 }}
                  >
                    <Typography variant='caption' sx={{ color: 'text.disabled' }}>
                      Avg. orders per rep
                    </Typography>
                  ...
                  >
                    <Typography variant='caption' fontWeight={600} sx={{ color: '#818cf8' }}>
                ...
                >
                  ...
                  <Box>
                    <Typography
                      variant='body2'
                      sx={{ color: 'text.primary', fontStyle: 'italic', mb: 1, lineHeight: 1.5, fontSize: '0.85rem' }}
                    >
                    ...
                    <Typography
                      variant='caption'
                      fontWeight={700}
                      sx={{ color: '#818cf8', fontSize: '0.65rem', letterSpacing: '0.5px' }}
                    >
                      — DAVID CHEN, SALES DIRECTOR
                    </Typography>
                  </Box>
                </Box>
              </Stack>
          ...
          <Box sx={{ bgcolor: 'action.hover', px: 4, py: 3, borderTop: '1px solid', borderColor: 'divider' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
              <Typography
                variant='caption'
                fontWeight={700}
                sx={{ color: '#c084fc', textTransform: 'uppercase', letterSpacing: '0.5px', fontSize: '0.65rem' }}
              >
                Incentive Tracking Preview
              </Typography>
            ...
            <Box sx={{ position: 'relative', height: 45, mb: 1, mt: 4 }}>
              ...
              <Box
                sx={{
                  ...
                  borderRadius: 3,
                  background:
                    'linear-gradient(90deg, rgba(251,146,60,0.1), rgba(192,132,252,0.1), rgba(129,140,248,0.1))'
                }}
              />
