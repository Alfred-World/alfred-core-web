      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
        ...
        {trendColor === 'success' ? (
          <Chip
            label={trend}
            size='small'
            sx={{ bgcolor: alpha('#10b981', 0.1), color: '#10b981', fontWeight: 700, borderRadius: 1.5 }}
            icon={<i className='tabler-trending-up' style={{ color: '#10b981', fontSize: 14 }} />}
          />
        ) : (
          <Chip
            label={trend}
            size='small'
            sx={{ bgcolor: 'action.hover', color: 'text.secondary', fontWeight: 600, borderRadius: 1.5 }}
          />
        )}
      </Box>
      <Box sx={{ mt: 'auto' }}>
        <Typography
          variant='caption'
          sx={{ color: 'text.disabled', fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase' }}
        >
          {title}
        </Typography>
...
const AccountSalesDashboard = () => {
  ...
  useEffect(() => {
    if (
      commissionSettingQuery.data?.success &&
      typeof commissionSettingQuery.data.result?.commissionPercent === 'number'
    ) {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, p: { xs: 1, md: 0 } }}>
      ...
      >
        <Box>
          ...
          <Typography variant='body2' color='text.secondary' sx={{ mt: 0.5 }}>
            Real-time operational performance and financial insights.
          </Typography>
        </Box>
      ...
      {/* Row 1: Stat Cards */}
      <Box
        sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', lg: 'repeat(4, 1fr)' }, gap: 3 }}
      >
        <StatCard
          title='TOTAL REVENUE'
          value={`$${(dashboardData.totalRevenue / 1).toLocaleString('en-US', { minimumFractionDigits: 2 })}`}
          icon='tabler-wallet'
          color='#818cf8'
          trend='+12.5%'
        />
        <StatCard
          title='TOTAL SALES'
          value={(ordersQuery.data?.result?.total ?? dashboardData.rows.length).toLocaleString('en-US')}
        ...
        <StatCard
          ...
          value={productData?.result?.total ? productData.result.total.toLocaleString('en-US') : '0'}
          icon='tabler-package'
          color='#fb923c'
          trend='Stable'
          trendColor='neutral'
        ...
        <StatCard
          title='TOTAL MEMBERS'
          value={memberData?.result?.total ? memberData.result.total.toLocaleString('en-US') : '0'}
          icon='tabler-users'
          color='#475569'
          trend='+24%'
        />
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 4 }}>
            <Box>
              ...
              <Typography variant='body2' color='text.secondary'>
                Monthly breakdown for the current fiscal year
              </Typography>
            </Box>
            <Button
              size='small'
              ...
              <BarChart data={mockChartData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                ...
                <Bar dataKey='value' radius={[4, 4, 4, 4]}>
                  {mockChartData.map((entry, index) => (
                    ...
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.name === 'JUN' ? '#a855f7' : alpha(theme.palette.text.primary, 0.08)}
                    />
                  ))}
                </Bar>
              </BarChart>
        ...
        >
          ...
          <Box
            sx={{
              ...
              p: 2,
              borderRadius: 2,
              bgcolor: alpha(sourceColor.Facebook, 0.08),
              border: '1px solid',
              borderColor: alpha(sourceColor.Facebook, 0.2),
              display: 'flex',
              gap: 1.5,
              alignItems: 'flex-start'
            }}
          >
            ...
            <Typography variant='body2' sx={{ color: 'text.secondary', lineHeight: 1.5 }}>
              <strong style={{ color: theme.palette.text.primary }}>Facebook</strong> conversions are up by 12% compared
              to last month.
            </Typography>
          </Box>
        </Card>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '8fr 4fr' }, gap: 3 }}>
        <Card sx={{ p: 0, borderRadius: 3, border: '1px solid', borderColor: 'divider' }}>
          ...
          >
            ...
            <Typography variant='body2' fontWeight={600} sx={{ color: '#818cf8', cursor: 'pointer' }}>
              View All Orders
            </Typography>
          </Box>
        ...
        <Card sx={{ p: 3, borderRadius: 3, border: '1px solid', borderColor: 'divider' }}>
          ...
          >
            <TextField
              ...
              slotProps={{
                ...
                input: { disableUnderline: true, sx: { px: 2, py: 1, fontWeight: 700 } }
              }}
              sx={{ flex: 1 }}
            />
            <Box
          ...
          <Typography
            variant='caption'
            fontWeight={700}
            sx={{ textTransform: 'uppercase', letterSpacing: 1, display: 'block', mb: 2, color: 'text.secondary' }}
          >
            Change History
          </Typography>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '7fr 5fr' }, gap: 3 }}>
        ...
        <Card
          sx={{
            ...
            bgcolor: alpha('#1e1b4b', 0.4),
            backgroundImage: 'linear-gradient(145deg, rgba(30, 27, 75, 0.4) 0%, rgba(13, 11, 38, 0.8) 100%)'
          }}
        >
          ...
          <Typography variant='body2' sx={{ color: 'text.secondary', lineHeight: 1.7, mb: 4 }}>
            &quot;Your sales funnel on <strong style={{ color: '#c084fc' }}>TikTok</strong> is outperforming other
            channels by 24% in engagement. Consider shifting $2,500 from the experimental budget to TikTok Ads to
            maximize the current viral trend.&quot;
          </Typography>
