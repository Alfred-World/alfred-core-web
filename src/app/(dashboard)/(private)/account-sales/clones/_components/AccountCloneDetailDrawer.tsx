      ...
      <Box sx={{ flex: 1, overflowY: 'auto', p: { xs: 3, sm: 4 }, display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <Stack direction='row' spacing={2}>
          <Card variant='outlined' sx={{ flex: 1, p: 2, borderRadius: 2 }}>
            <Typography variant='caption' sx={{ color: 'primary.main', fontWeight: 700, mb: 1, display: 'block', letterSpacing: '1px' }}>
              SOURCE ACCOUNT
            </Typography>
        ...
        {/* Access Credentials */}
        <Box>
          <Typography variant='caption' sx={{ display: 'flex', alignItems: 'center', gap: 1, fontWeight: 700, mb: 2, letterSpacing: '1px' }}>
            <Box component='span' sx={{ width: 4, height: 4, borderRadius: '50%', bgcolor: 'primary.main' }} />
            ACCESS CREDENTIALS
          </Typography>
          <Stack spacing={2}>
            {/* Username Box */}
            <Box sx={{ bgcolor: 'background.default', borderRadius: 2, border: '1px solid', borderColor: 'divider', p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Box>
                <Typography variant='caption' sx={{ color: 'text.secondary', mb: 0.5, display: 'block' }}>USERNAME</Typography>
                <Typography variant='body2' sx={{ fontWeight: 600, fontFamily: 'monospace', fontSize: '14px' }}>
                  {selectedClone?.username}
                </Typography>
              </Box>
              <IconButton size='small' onClick={() => handleCopy(selectedClone?.username)} sx={{ color: 'text.secondary' }}>
            ...
            <Box sx={{ bgcolor: 'background.default', borderRadius: 2, border: '1px solid', borderColor: 'divider', p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Box>
                <Typography variant='caption' sx={{ color: 'text.secondary', mb: 0.5, display: 'block' }}>PASSWORD</Typography>
              ...
              <Stack direction='row' spacing={1}>
                ...
                <IconButton size='small' onClick={() => handleCopy(selectedClone?.password)} sx={{ color: 'text.secondary' }}>
                  <i className='tabler-copy' style={{ fontSize: 18 }} />
                </IconButton>
              </Stack>
            </Box>
        ...
        <Box>
          ...
          >
            ...
            {otpView.valid && (
              <Box sx={{ position: 'absolute', top: -50, right: -50, width: 150, height: 150, background: `radial-gradient(circle, ${alpha(theme.palette.primary.main, 0.2)} 0%, transparent 70%)`, borderRadius: '50%', pointerEvents: 'none' }} />
            )}

            <Stack direction='row' spacing={3} alignItems='center'>
              {/* QR Code pseudo box */}
              {otpauthUri ? (
                <Box sx={{ p: 1, bgcolor: '#fff', borderRadius: 2, width: 96, height: 96, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <QRCodeSVG value={otpauthUri} size={84} style={{ display: 'block' }} />
                </Box>
              ) : (
                <Box sx={{ width: 96, height: 96, bgcolor: 'background.default', borderRadius: 2, border: '1px solid', borderColor: 'divider', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <Box sx={{ flex: 1 }}>
                <Typography variant='caption' sx={{ color: 'text.secondary', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 1 }}>
                  LIVE OTP CODE
                  {otpView.valid && (
                    <Box component='span' sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: 'success.main', boxShadow: theme => `0 0 8px ${alpha(theme.palette.success.main, 0.8)}` }} />
                  )}
                </Typography>
                <Stack direction='row' alignItems='center' spacing={2}>
                  ...
                  {otpView.valid && (
                    <IconButton size='small' onClick={() => handleCopy(otpView.code)} sx={{ color: 'text.secondary', bgcolor: 'background.default', borderRadius: 1.5, border: '1px solid', borderColor: 'divider', flexShrink: 0 }}>
                ...
                <Box sx={{ mt: 1.5 }}>
                  <Typography variant='caption' sx={{ color: 'text.secondary', mb: 0.5, display: 'block' }}>2FA SECRET</Typography>
                  <Stack direction='row' alignItems='center' spacing={1}>
                    <Box sx={{ bgcolor: 'background.default', px: 1.5, py: 0.5, borderRadius: 1.5, border: '1px solid', borderColor: 'divider', flex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography variant='caption' sx={{ fontFamily: 'monospace' }}>
                        {showSecret ? selectedClone?.twoFaSecret : maskSensitiveValue(selectedClone?.twoFaSecret)}
                      </Typography>
                      <IconButton size='small' onClick={() => setShowSecret(!showSecret)} sx={{ p: 0.25, color: 'text.secondary' }}>
                        <i className={showSecret ? 'tabler-eye-off' : 'tabler-eye'} style={{ fontSize: 14 }} />
                      </IconButton>
                    </Box>
                    <IconButton size='small' onClick={() => handleCopy(selectedClone?.twoFaSecret)} sx={{ color: 'text.secondary', bgcolor: 'background.default', borderRadius: 1.5, border: '1px solid', borderColor: 'divider' }}>
                      <i className='tabler-copy' style={{ fontSize: 16 }} />
                    </IconButton>
                  </Stack>
                </Box>
            ...
            {otpView.valid && (
              <Box sx={{ position: 'absolute', bottom: 0, left: 0, height: 2, bgcolor: 'primary.main', width: `${((OTP_STEP_SECONDS - otpView.remainingSeconds) / OTP_STEP_SECONDS) * 100}%`, transition: 'width 1s linear' }} />
            )}
          </Card>
        </Box>
        ...
        <Box>
          <Typography variant='caption' sx={{ color: 'text.secondary', fontWeight: 700, mb: 1, display: 'block', letterSpacing: '1px' }}>
            RECENT ACTIVITY
          </Typography>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 1 }}>
            <Typography variant='caption' sx={{ color: 'text.secondary', fontSize: '0.65rem', textTransform: 'uppercase' }}>LAST CLONED: 2H AGO</Typography>
            <Typography variant='caption' sx={{ color: 'text.secondary', fontSize: '0.65rem' }}>STABILITY: 99.8%</Typography>
          </Box>
        </Box>
      </Box>

      {/* Footer Actions */}
      <Box sx={{ p: 3, borderTop: '1px solid', borderColor: 'divider', bgcolor: 'background.paper', display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
