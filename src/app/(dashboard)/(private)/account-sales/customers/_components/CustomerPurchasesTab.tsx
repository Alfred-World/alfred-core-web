    ...
    <Box>
        ...
        <Box sx={{ p: 2.5, bgcolor: 'action.hover', border: '1px solid', borderColor: 'divider', borderRadius: 3 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <Box>
              ...
              <Typography variant='caption' sx={{ color: '#10b981', fontWeight: 700, display: 'flex', alignItems: 'center' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Button size='small' variant='outlined' startIcon={<i className='tabler-download' />}>Export</Button>
          <Button size='small' variant='outlined' startIcon={<i className='tabler-filter' />}>Filter</Button>
        </Box>
      </Box>
      ...
      <TableContainer sx={{ bgcolor: 'transparent', border: '1px solid', borderColor: 'divider', borderRadius: 3 }}>
        <Table>
          ...
          <TableBody>
            {orders.map(order => {
              const statusColor = order.status === AccountOrderStatus.Active ? 'success' : order.status === AccountOrderStatus.Refunded ? 'warning' : 'default'
              const statusHex = statusColor === 'success' ? '#10b981' : statusColor === 'warning' ? '#f59e0b' : '#94a3b8'
                  ...
                  <TableCell>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                      <Box sx={{ width: 32, height: 32, borderRadius: 1.5, bgcolor: 'action.hover', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <i className={order.productName?.includes('Bundle') ? 'tabler-packages' : 'tabler-box'} style={{ color: 'text.secondary', fontSize: 16 }} />
                      </Box>
                      <Box>
                        <Typography variant='body2' fontWeight={700}>{order.productName}</Typography>
                        <Typography variant='caption' sx={{ color: 'text.secondary' }}>{order.productVariantNameSnapshot}</Typography>
                      </Box>
                    </Box>
                  </TableCell>
                  <TableCell sx={{ color: 'text.secondary' }}>{order.purchaseDate?.slice(0, 10)}</TableCell>
                  <TableCell sx={{ fontWeight: 700 }}>{(order.unitPriceSnapshot ?? 0).toLocaleString('vi-VN')}₫</TableCell>
                  <TableCell>
                    <Chip
                      size='small'
